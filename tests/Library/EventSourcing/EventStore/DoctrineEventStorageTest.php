...
class DoctrineEventStorageTest extends DoctrineTestCase
{
	...
	public function test_it_loads_stream_with_3_events()
	{
		...
		$result = $storage->loadStream(new EntityVersionData('Entity', new Id(1)));
	...
	{
		$storage = new DoctrineEventStorage($this->em);
		$result = $storage->loadStream(new EntityVersionData('Other', new Id(1)));
	...
	{
		$storage = new DoctrineEventStorage($this->em);
		$result = $storage->loadStream(new EntityVersionData('Entity', new Id(2)));
	...
	{
		$storage = new DoctrineEventStorage($this->em);
		$this->assertEquals(3, $storage->count(new EntityVersionData('Entity', new Id(1), 0)));
		$this->assertEquals(4, $storage->count(new EntityVersionData('Other', new Id(1), 0)));
		$this->assertEquals(6, $storage->count(new EntityVersionData('Entity', new Id(2), 0)));
	}
	...
	public function test_it_can_store_a_stream()
	{
		...
		$this->assertEquals(5, $storage->count(new EntityVersionData('Entity', new Id(3), 0)));
	}
	...
	public function test_it_can_save_a_stream_and_load_it_and_remains_equal()
	{
		...
		$stream = $this->prepareEventStream('Entity', new Id(3), 5);
		$storage->saveStream($stream);
		$loaded = $storage->loadStream(new EntityVersionData('Entity', new Id(3)));
