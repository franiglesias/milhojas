...
class ObjectPropertyDescriptorTest extends AbstractPropertyDescriptorTest {
	...
	{
		$pd = new ObjectPropertyDescriptor($this->getProperty('member'), $this->getClass());
		$this->assertEquals(array('member.id' => 1, 'member.content' => 'Content'), $pd->describe());
	}

	public function test_it_describes_empty_property_as_class_name_with_prefix()
	{
		$pd = new ObjectPropertyDescriptor($this->getProperty('member'),  $this->getClass());
		$this->assertEquals(array('prefix.member.id' => 1, 'prefix.member.content' => 'Content'), $pd->describe('prefix'));
	}
}
