...
* Description
*/
class PublicationDateRangeTest extends \PHPUnit_Framework_Testcase
{
	# Basic usage
	
	public function test_it_returns_true_if_date_is_in_the_interval_including_limits()
	{
		$Pub = new PublicationDateRange(new \DateTimeImmutable(), new \DateTimeImmutable('+5 day'));
	...
	{
		$Pub = new PublicationDateRange(new \DateTimeImmutable());
	...
	{
		$PubDate = new PublicationDateRange(null);
	}
	...
	{
		$PubDate = new PublicationDateRange(null, new \DateTimeImmutable());
	}
	...
	{
		$PubDate = new PublicationDateRange(new \DateTimeImmutable(), new \DateTimeImmutable());
	}
	/**
	 * @expectedException InvalidArgumentException
	 */
	...
	{
		$PubDate = new PublicationDateRange(new \DateTimeImmutable(), new \DateTimeImmutable('-5 day'));
	}
	...
	public function test_change_expiration_returns_object()
	{
		...
		$this->assertInstanceOf('\Library\ValueObjects\Dates\PublicationDateRange', $NewPub);

		$Expected = new PublicationDateRange(new \DateTimeImmutable(), new \DateTimeImmutable('+ 5 day'));
		$this->assertEquals($Expected, $NewPub);
	}
	
	public function test_can_change_expiration_to_null_value()
	{
		$Pub = new PublicationDateRange(new \DateTimeImmutable(), new \DateTimeImmutable('+2 day'));
		$NewPub = $Pub->changeExpiration();
		$this->assertInstanceOf('\Library\ValueObjects\Dates\PublicationDateRange', $NewPub);

		$Expected = new PublicationDateRange(new \DateTimeImmutable());
		$this->assertEquals($Expected, $NewPub);
	}
	...
	public function test_change_start_returns_object()
	{
		...
		$Expected = new PublicationDateRange(new \DateTimeImmutable('+ 5 day'));
		$this->assertEquals($Expected, $NewPub);
	}
